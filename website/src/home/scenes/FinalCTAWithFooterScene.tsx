...
export const FinalCTAWithFooterScene = ({ active, reducedMotion, theme, onNavigate }: SceneComponentProps) => {
  ...
  return (
    <div className="final-cta-scene relative grid h-full max-h-full min-h-0 w-full grid-rows-[minmax(0,1fr)_auto] gap-4 overflow-hidden px-4 pb-4 pt-4 sm:px-8 sm:pb-5 sm:pt-5 lg:px-16 lg:pb-6 lg:pt-6">
      {/* ── Hero finale — sized to fit 100% zoom (nav + backdrop + footer) ── */}
      <div className="final-cta-body relative grid min-h-0 items-center gap-8 lg:grid-cols-[minmax(0,1.05fr)_minmax(min(440px,100%),1.35fr)] lg:gap-10 xl:grid-cols-[minmax(0,1fr)_minmax(min(460px,100%),1.45fr)]" dir="ltr">
        {/* Explicit left column: phone artwork */}
        <div className="final-cta-art relative flex min-h-0 min-w-0 items-end justify-center pb-2 lg:max-h-[min(82svh,820px)] lg:justify-start lg:pb-0">
          <div
        ...
        <div dir="rtl" className="final-cta-copy flex min-w-0 flex-col items-center justify-center px-2 text-center sm:px-4 lg:min-h-[min(70svh,640px)] lg:px-6 lg:py-4 xl:min-h-[min(72svh,680px)]">
          {/* INSERT COIN prompt */}
          <div
            className={`final-cta-coin mb-3 ${active ? 'kinetic-fade' : 'opacity-0'}`}
          ...
          <h2
            className="final-cta-heading mx-auto max-w-[22ch] font-black leading-[1.05] tracking-tight font-cartoon sm:max-w-[26ch]"
          ...
          <p
            className={`final-cta-description mx-auto mt-5 max-w-md text-base leading-relaxed sm:max-w-lg sm:text-[1.06rem] sm:leading-8 font-cartoon md:max-w-2xl ${active ? 'kinetic-rise' : 'opacity-0'}`}
          ...
          <div
            className={`final-cta-actions mt-8 flex flex-wrap items-center justify-center gap-4 ${active ? 'kinetic-rise' : 'opacity-0'}`}
            style={{ animationDelay: '950ms' }}
          >
            {/* GAME START button */}
            <button
              type="button"
              className="final-cta-action magnetic-cta inline-flex items-center justify-center px-7 py-3 font-cartoon text-sm font-bold transition-all sm:px-9 sm:text-base sm:py-3.5"
              style={{
            ...
            <button
              ...
              className="final-cta-action inline-flex items-center justify-center gap-2 px-5 py-3 font-cartoon text-sm font-bold transition-all hover:brightness-110 sm:px-6 sm:py-3.5 sm:text-base"
              style={{
      ...
      <footer className="final-cta-footer relative z-10 shrink-0 pt-3 text-right" style={{ borderTop: `1px solid ${theme.accent}22` }}>
        {/* Top neon line */}
        <div className="mb-2 h-px w-full" style={{ background: `linear-gradient(90deg, transparent, ${theme.accent}44, transparent)` }} />

        <div className="final-cta-footer-grid grid gap-4 items-center lg:grid-cols-[minmax(auto,1fr)_minmax(auto,1fr)_minmax(auto,1fr)]">
          <div className="final-cta-footer-brand flex items-center justify-start gap-3">
            ...
            <img
              src="/New_Logo.png"
              alt="شعار YouSafe"
              className="final-cta-footer-logo h-10 w-auto"
              style={{ filter: `drop-shadow(0 0 6px ${theme.glow})` }}
            />
          </div>

          <div
            className="final-cta-footer-links flex flex-wrap items-center justify-center gap-3 font-cartoon text-[9px]"
            style={{
              color: '#ffffff',
              letterSpacing: '0.1em',
              opacity: 0.95,
          ...
          <div className="final-cta-email-wrap flex items-center justify-end">
            <span
              className="final-cta-email inline-flex items-center gap-2 px-4 py-2 text-[0.82rem] font-bold font-cartoon"
              style={{
